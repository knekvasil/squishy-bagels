...
def load_model(ckpt_path, device):
    ...
    print(f"...(load_model) Model initialized: {scorenet}")
    print(f"...(load_model) Model parameters: {list(scorenet.parameters())}")
...
def get_model(config):
    ...
    print(f"Config Model Version: {version}, Arch: {arch}, Depth: {depth}")
    if arch == "unetmore":
...
class NCSNRunner:
    ...
    def get_mode(self):
        self.condf, self.condp = (
            self.config.data.num_frames_cond,
            getattr(self.config.data, "prob_mask_cond", 0.0),
        )
        self.futrf, self.futrp = (
            getattr(self.config.data, "num_frames_future", 0),
    ...
    def train(self):
        ...
        for name, param in scorenet.named_parameters():
                ...
                if (
                    ...
                    if conditional:
                        ...
                        for t in range(condi.shape[1] // self.config.data.channels):
                            cond_t = condi[
                                :,
                                t * self.config.data.channels : (t + 1)
                                * self.config.data.channels,
                            ]  # BCHW
                            frame = torch.cat(
                                [
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        for i, (real_, _) in tqdm(
            ...
            if calc_fvd1 or calc_fvd2 or calc_fvd3:
                ...
                if (calc_fvd1 or (calc_fvd3 and not second_calc)) and real.shape[
                    ...
                    if future == 0:
                        real_fvd = torch.cat(
                            ...
                            dim=1,
                        )[::preds_per_test]  # Ignore the repeated ones
                    else:
                        real_fvd = torch.cat(
                            [
                                cond_original[
                ...
                if (second_calc and (calc_fvd2 or calc_fvd3)) and real.shape[
                    1
                ] >= pred.shape[1]:  # only cond, but real has all frames req for interp
                    # real2
                    real_fvd2 = torch.cat(
                        [
                            cond_original2[
        ...
        def fvd_stuff(fake_embeddings, real_embeddings):
            ...
            if preds_per_test > 1:
                ...
                fvd_traj_mean, fvd_traj_std = (
                    float(np.mean(fvds_list)),
                    float(np.std(fvds_list)),
                )
